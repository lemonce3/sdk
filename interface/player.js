...
module.exports = class ActionQueue {
	...
	$then(nextTick) {
		...
		this.promise = this.promise.then(nextTick);

		return this;
	}

	goto(URL) {
		return this.$then(() => this.native.goto(URL));
	}
	...
	select(selector, index) {
		return this.$then(() => this.driver.selectByIndex(selector, index));
	}
	...
	upload(fileList) {
		return this.$then(() => this.native.upload(fileList));
	}
